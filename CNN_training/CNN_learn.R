...
decompose_2D_to_1D_training <- function(rr_norm,
  ...
  # OPTIMISÉ : Toutes les lignes avec pics + 1/10 des autres
  # ============================================================
  
  if (extract_F1) {
    cat("Extraction des coupes horizontales (lignes, axe F1)...\n")
    
    # Identifier les indices de lignes contenant au moins un pic
    rows_with_peaks <- sapply(peaks_2D$F2_ppm, function(p) which.min(abs(F2_axis - p)))
    rows_with_peaks <- unique(rows_with_peaks)
    
    # Sous-échantillonner les lignes SANS pics (1 sur 4 = 25%)
    all_rows <- seq_len(nrow(rr_norm))
    rows_without_peaks <- setdiff(all_rows, rows_with_peaks)
    rows_subsampled <- rows_without_peaks[seq(1, length(rows_without_peaks), by = 4)]
    
    # Combiner : toutes les lignes avec pics + sous-échantillon des autres
    rows_to_extract <- sort(unique(c(rows_with_peaks, rows_subsampled)))
    
    cat(sprintf("  -> %d lignes avec pics + %d lignes sous-échantillonnées = %d total (sur %d)\n", 
                length(rows_with_peaks), length(rows_subsampled), 
                length(rows_to_extract), nrow(rr_norm)))
    
    if (length(rows_to_extract) > 0) {
      pb <- txtProgressBar(min = 0, max = length(rows_to_extract), style = 3)
      
      for (iter in seq_along(rows_to_extract)) {
        i <- rows_to_extract[iter]
        F2_current <- F2_axis[i]
        slice_1D <- rr_norm[i, ]
        
        # Pics sur cette ligne (F2 ~ F2_current)
        peaks_on_slice <- peaks_2D %>%
          dplyr::filter(abs(F2_ppm - F2_current) <= ppm_tolerance_F2)
  ...
  if (extract_F2) {
    ...
    # Identifier les indices de colonnes contenant au moins un pic
    cols_with_peaks <- sapply(peaks_2D$F1_ppm, function(p) which.min(abs(F1_axis - p)))
    cols_with_peaks <- unique(cols_with_peaks)
    
    # Sous-échantillonner les colonnes SANS pics (1 sur 4 = 25%)
    all_cols <- seq_len(ncol(rr_norm))
    cols_without_peaks <- setdiff(all_cols, cols_with_peaks)
    cols_subsampled <- cols_without_peaks[seq(1, length(cols_without_peaks), by = 4)]
    
    # Combiner : toutes les colonnes avec pics + sous-échantillon des autres
    cols_to_extract <- sort(unique(c(cols_with_peaks, cols_subsampled)))
    
    cat(sprintf("  -> %d colonnes avec pics + %d colonnes sous-échantillonnées = %d total (sur %d)\n", 
                length(cols_with_peaks), length(cols_subsampled), 
                length(cols_to_extract), ncol(rr_norm)))
    
    if (length(cols_to_extract) > 0) {
      pb <- txtProgressBar(min = 0, max = length(cols_to_extract), style = 3)
      
      for (iter in seq_along(cols_to_extract)) {
        j <- cols_to_extract[iter]
        F1_current <- F1_axis[j]
        slice_1D <- rr_norm[, j]
        
        # Pics sur cette colonne (F1 ~ F1_current)
        peaks_on_slice <- peaks_2D %>%
          dplyr::filter(abs(F1_ppm - F1_current) <= ppm_tolerance_F1)
    ...
    n_cols_extracted <- length(cols_to_extract)
    cat(sprintf("%d coupes verticales (%s) extraites\n", n_cols_extracted, nucleus_F2))
  } else {
    # Si extract_F2 est FALSE, initialiser les variables
    n_cols_extracted <- 0
  }
