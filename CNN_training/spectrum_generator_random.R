...
generate_random_spectrum_with_peaks <- function(BdD = BdDReference_CPMG,
  ...
  couplage_to_n <- function(c) {
    # Gérer les cas NA, NULL, vide ou vecteurs multiples
    if (is.null(c) || length(c) == 0 || is.na(c[1]) || c[1] == "") {
      return(0)
    }
    # Prendre seulement le premier caractère si plusieurs
    c <- as.character(c[1])
  ...
  for (i in seq_along(chosen)) {
    ...
    for (j in seq_len(nrow(groups))) {
      ...
      if (is.null(grp[["ppm"]]) || length(grp[["ppm"]]) == 0 || is.na(grp[["ppm"]])) {
        next  # Sauter cette ligne
      }
      if (is.null(grp[["nH"]]) || length(grp[["nH"]]) == 0 || is.na(grp[["nH"]])) {
        next
      }
      ...
      all_peaks[[length(all_peaks) + 1]] <- data.frame(
        ...
        ppm = grp[["ppm"]],
        nH = grp[["nH"]],
        couplage = as.character(grp[["couplage"]]),
        n_couplings = n_couplings,
