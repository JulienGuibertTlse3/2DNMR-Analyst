                       ...
                       tags$details(
                         ...
                         div(
                           ...
                           conditionalPanel("input.seuil_method == 'max_pct'",
                                            numericInput("pct_val", "Percentage:", value = 0.01, min = 0.001, max = 1, step = 0.001)
                           ),
                           conditionalPanel("input.seuil_method == 'bruit_mult'",
                                            numericInput("bruit_mult", "Multiplier:", value = 3, min = 0.5, max = 10, step = 0.5)
                           )
                         )
                       ),
                     ...
                     bsCollapsePanel(
                       ...
                       tags$details(
                         ...
                         div(
                           ...
                           textAreaInput("keep_peak_ranges_text", "Keep ranges:", 
...
server <- function(input, output, session) {
  ...
  observeEvent(input$generate_plot, {
    ...
    for (i in seq_along(spectra_list())) {
      ...
      result <- tryCatch({
        ...
        showNotification(paste("❌ Error:", names(spectra_list())[i], "-", e$message), type = "error")
        NULL
      })
    ...
    names(all_plots) <- names(spectra_list())
    ...
    if (length(all_results) > 0 && !is.null(all_results[[1]])) {
      ...
      refresh_nmr_plot()
      showNotification("✅ Plots generated", type = "message")
  ...
  observeEvent(input$generate_centroids, {
    ...
    status_msg("🔄 Generating centroids...")
    ...
    showNotification("✅ Peaks generated", type = "message")
    status_msg("✅ Peak picking complete")
    shinyjs::hide("loading_message")
  })
  ...
  observeEvent(input$generate_centroids_cnn, {
    ...
    if (!is.null(result_peaks$boxes) && nrow(result_peaks$boxes) > 0) {
      result_peaks$peaks <- result_peaks$boxes %>%
        dplyr::transmute(
          F1 = as.integer(round(cx_ppm)), F2 = as.integer(round(cy_ppm)),
          F1_ppm = cx_ppm, F2_ppm = cy_ppm,
    ...
    showNotification("✅ CNN peak picking complete", type = "message")
    status_msg("✅ CNN peak picking complete")
    shinyjs::hide("loading_message")
  })
  ...
  # Déplacer la box (shift par delta)
  ...
  observeEvent(input$import_boxes_file, {
    ...
    imported <- tryCatch(
      read.csv(input$import_boxes_file$datapath, sep = ";", stringsAsFactors = FALSE),
      error = function(e) {
    ...
    if (is.null(imported) || !all(required_cols %in% colnames(imported))) {
      showNotification(
        paste(
    ...
    imported <- imported[, required_cols]
    
    # Reorder or enforce numeric types
    imported$stain_id <- as.integer(imported$stain_id)
    ...
    showNotification("✅ Bounding boxes imported", type = "message")
  })
  ...
  output$toolDescription <- renderUI({
    tags$div(
      ...
      div(style = "background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 10px; margin-bottom: 20px;",
          h2(style = "margin: 0;", "🔬 2D NMR Spectra Analysis Tool"),
          p(style = "margin: 10px 0 0 0; opacity: 0.9;", "Interactive analysis and quantification of 2D NMR spectra")
      ),
  ...
  output$interactivePlot <- renderPlotly({
    ...
    tryCatch({
      ...
      p <- ggplotly(plot_obj, source = "nmr_plot") %>%
      ...
      if (!is.null(x_range) && !is.null(y_range)) {
        ...
        p <- p %>%
          add_trace(x = grid$x, y = grid$y, type = "scatter", mode = "markers",
                    marker = list(size = 8, opacity = 0),  # Taille réduite pour meilleure précision
